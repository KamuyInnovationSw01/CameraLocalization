# カメラローカライゼーション - マーカー／マーカーレス ポーズ推定

USBカメラでARマーカー（ArUco）または特定の剛体被写体を撮影し、カメラの3次元姿勢（位置・向き）をリアルタイムで3Dワイヤフレーム表示するアプリケーションです。
...
- **再投影誤差による姿勢推定結果の確認**
- **ALIKED + LightGlueによるマーカーレスローカライゼーション**
- **参照画像から簡易3Dマップを生成する独立CLI**
- **マッチング特徴点の状態表示**（緑: インライア、赤: 外れ値、黄: 未検証）
...
- `markerless_features.py`: ALIKED／LightGlueの遅延初期化と特徴処理
- `build_markerless_map.py`: 参照画像から簡易3Dマップを生成する準備ソフトウェア
- `markerless_localizer.py`: 保存済み3Dマップを使う実行時ローカライザー
- `REFERENCE_IMAGES_GUIDE.md`: 参照画像撮影、マップ生成、座標系の手順
...
| `numpy` | 数値計算・行列演算 |
| `PyYAML` | 設定ファイル読み込み |
| `lightglue` | マーカーレス方式のALIKED特徴量とLightGlue対応付け |
| `torch` | ALIKED／LightGlueのCPU・CUDA実行基盤 |

マーカーレス方式を使わない場合も、`lightglue`と`torch`はrequirements.txtに含まれます。CUDAを使う場合は、対象GPUとCUDAに対応したPyTorchパッケージを選択してください。
...
- **マーカー方式**: マーカー（赤い正方形 + 座標軸）を原点に配置
- **マーカーレス方式**: マーカー四角形の代わりに簡易3Dマップを表示
- **カメラ本体**（オレンジの長方体）：カメラ位置と向きを表現
- **フラスタム**（シアン）：カメラ前方の視錐台
- **視線方向**（黄色矢印）：カメラ光軸（マーカーへの方向）
- **座標軸**（赤=X, 緑=Y, 青=Z）：マーカー方式で表示
- **マップ点の色**（マーカーレス方式）: 灰=未対応、黄=未検証、赤=外れ値、緑=インライア
- **自動フィット**：表示対象全体を検出し、上下左右に余白を確保して画面内に表示

3DビューはOpenCVの`cv2.projectPoints`による透視投影を使います。カメラのフラスタムは
キャリブレーションの`fx`、`fy`、`cx`、`cy`と画像サイズから計算します。表示上のパースが強く
なりすぎないよう、仮想ビューカメラはシーンの大きさに応じて後退させます。
...
3Dビューの仮想カメラは右手系LookAtで構築します。マーカー方式ではマーカーの`+Z`正面方向を
画面上向きに表示します。マーカーレス方式では表示専用にX軸まわり180度の回転を適用し、
基準座標系の`+Z`が画面下向きになるように表示します。保存済みのマップ座標や推定値自体は変更しません。

姿勢推定に失敗したフレームでは、3Dビューは更新せず暗い背景を表示します。対応点はカメラ映像
側に引き続き表示されます。
...
- **描画バックエンド**: OpenCV（`cv2.projectPoints`による高速描画）
